...
df.ever_married = df.ever_married.replace(to_replace = ['Yes','No'], value = [1,2])
...
gender = {k:v for v, k in enumerate(df.gender.unique())}
df['gender'] = df['gender'].map(gender)
work_t= {k:v for v, k in enumerate(df.work_type.unique())}
df['work_type'] = df['work_type'].map(work_t)

resident= {k:v for v, k in enumerate(df.Residence_type.unique())}
df['Residence_type'] = df['Residence_type'].map(resident)
smoking_status = {k:v for v, k in enumerate(df.smoking_status.unique())}
